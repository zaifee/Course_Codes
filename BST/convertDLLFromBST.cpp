...
// Convert this BST INTO doubly sorted linkedlist 
void createDoublyLinkedListFromBST(Node* root, Node* &head){
    if(root == NULL){
        return;
    }
    ...
    // R 
    createDoublyLinkedListFromBST(root->right, head);
    
    // N 
    // attaching next pointer of curr node to head of ll
    root->right = head;
    //left hai ya nhi ye check karlo 
    if(head != NULL) 
    ...
    //L 
    createDoublyLinkedListFromBST(root->left, head);
...
Node* head = NULL;
createDoublyLinkedListFromBST(root, head);
